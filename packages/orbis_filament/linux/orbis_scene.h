...
//
// The Linux counterpart to ../android/.../OrbisScene.kt, which mirrors
// OrbisFilamentPlugin.swift's private `Scene` struct -- same fields, same
// order, same optionality, so a change made to one is easy to find in the
// others. Like Kotlin's, this checks that the arrays a call reads are the
// *shape* the renderer needs (present when required, parallel arrays the
// same length) and trusts the indices inside them, because the one place
// these numbers come from is Dart's own OrbisScene.toMessage().
//
// What is different here, and is the only real work this file does that the
// Kotlin one did not have to: the standard codec hands Kotlin typed arrays
// already (a Dart Float32List *is* a FloatArray), while on Linux it hands
// back an FlValue that has to be asked its type and then unwrapped. So every
// field below goes through one of the small borrowers in the .cc, each of
// which answers "absent, or the wrong type" as an empty array rather than as
// a failure -- which is exactly what the ABI's own "a null pointer and a
...
// because an FlValue passed to a method-call handler is alive for the whole
// of that call and the scene is applied inside it. Empty is (nullptr, 0),
// which is what every optional ABI array takes for "nothing to say".
template <typename T>
struct List {
