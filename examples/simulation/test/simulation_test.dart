...
void main() {
  test('a hierarchy moves with its parent', () {
    ...
    expect(worldPositionOf(sim.world, sim.transforms, body)[0], closeTo(2, 1e-4));
    final child = worldPositionOf(sim.world, sim.transforms, carried);
    expect(child[0], closeTo(2, 1e-4),
  ...
  test('the whole slice composes: simulate, propagate, replicate', () async {
    ...
    final netHost = NetHost(
        world: host.world, set: host.replication, networkId: host.networkId);
    ...
    final replicaBody = worldPositionOf(
        replica.world, replica.transforms, netClient.entityFor(bodyId)!);
    ...
    final replicaCarried = worldPositionOf(
        replica.world, replica.transforms, netClient.entityFor(carriedId)!);
    expect(replicaCarried[0], closeTo(hostCarried[0], 1e-4));
    expect(replicaCarried[1], closeTo(1, 1e-4),
        reason: 'the attachment arrives already in world space, with no '
            'parent link on the client at all');

    expect(replica.world.has(netClient.entityFor(carriedId)!,
            replica.transforms.parent), isFalse,
        reason: 'hierarchy is the authority\'s business; a replica needs the '
            'result, not the structure');
  });

  test('a thousand bodies step and replicate without per-entity calls', () async {
    final host = Simulation();
    final replica = Simulation();
    addTearDown(host.dispose);
    addTearDown(replica.dispose);
    ...
    // ignore: avoid_print
    print('  $count bodies, 10 ticks of simulate + propagate + replicate: '
